...
export function InlineAvailabilityCalendar() {
  const { t, language } = useLanguage()
  const { setBookingDates } = useBooking()
  const { unavailableDates, isDateUnavailable } = useAvailabilityDates()
  ...
  // 当前月日历网格：前导空位 + 1..daysInMonth，末尾补空位到整周
  const grid = useMemo(() => {
