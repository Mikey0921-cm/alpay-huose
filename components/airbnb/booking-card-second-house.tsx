...
export function BookingCardSecondHouse() {
  const { t, language } = useLanguage()
  const { currency } = useCurrency()
  const { checkIn, checkOut, nights, totalUsd, totalTry, setBookingDates } = useBookingSecondHouse()
  ...
  const tryClosePaymentModal = () => {
    ...
    if (hasProgress) setConfirmCloseOpen(true)
          ...
          <div className="relative">
            ...
            {guestsOpen && (
              <div className="absolute left-0 right-0 top-full z-10 rounded-b-xl border border-t-0 border-border bg-background p-4 shadow-lg">
                {[
                  { label: t("booking.adults"), desc: t("booking.age13Plus"), value: guests, min: 1, max: 2, key: "adults" },
                  { label: t("booking.children"), desc: t("booking.ages2to12"), value: 0, min: 0, max: 1, key: "children" },
                  { label: t("booking.infants"), desc: t("booking.under2"), value: 0, min: 0, max: 1, key: "infants" },
                ].map((group) => (
                  <div
                    key={group.key}
                ...
                <p className="mt-3 text-xs leading-relaxed text-muted-foreground">
                  {t("booking.guestsAndPetsNoteMax2")}
                </p>
                <button
...
{/* Reserve Button */}
      ...
      {/* Payment Modal (portal for mobile) */}
            ...
            {confirmCloseOpen && (
              <div className="absolute inset-0 z-20 flex items-center justify-center rounded-2xl bg-foreground/40 p-4">
                <div className="w-full max-w-sm rounded-xl border border-border bg-background p-6 shadow-xl">
                  <p className="mb-4 text-sm text-foreground">{t("payment.confirmLeave")}</p>
                  <div className="flex gap-3">
                    <button type="button" onClick={() => setConfirmCloseOpen(false)} className="flex-1 rounded-lg border border-border bg-background px-4 py-2.5 text-sm font-medium text-foreground hover:bg-secondary">{t("payment.stay")}</button>
                    <button type="button" onClick={closePaymentModal} className="flex-1 rounded-lg bg-foreground px-4 py-2.5 text-sm font-medium text-background hover:opacity-90">{t("payment.leave")}</button>
                  </div>
                </div>
              </div>
            )}
