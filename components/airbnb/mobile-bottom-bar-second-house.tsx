...
export function MobileBottomBarSecondHouse() {
  const { t } = useLanguage()
  const { currency } = useCurrency()
  const { checkIn, checkOut, totalUsd, totalTry, nights } = useBookingSecondHouse()
    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-between border-t border-border bg-background px-4 py-3 lg:hidden">
      <div>
        ...
        <div className="flex items-center gap-1 text-xs">
          ...
          <span className="font-medium text-foreground">4.59</span>
